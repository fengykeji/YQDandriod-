...
public class CityAdapter extends BaseAdapter {
    ...
    public CityAdapter(Context context, List<CityBean.DataBean> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            String currentStr = list.get(i).getCity_code();
            String previewStr = (i - 1) >= 0 ? list.get(i - 1).getCity_code()
                    : " ";
            if (!previewStr.equals(currentStr)) {//前一个首字母与当前首字母不同时加入HashMap中同时显示该字母
                String name = list.get(i).getCity_code();
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        String currentStr = list.get(position).getCity_code();
