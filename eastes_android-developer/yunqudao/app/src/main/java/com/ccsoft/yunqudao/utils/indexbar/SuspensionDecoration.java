package com.ccsoft.yunqudao.utils.indexbar;
...
import com.ccsoft.yunqudao.utils.indexbar.bean.ISuspensionInterface;
...
public class SuspensionDecoration extends RecyclerView.ItemDecoration {
    ...
    private static int COLOR_TITLE_BG = Color.parseColor("#F1F2F7");
    ...
    public SuspensionDecoration(Context context, List<? extends ISuspensionInterface> datas) {
        ...
        mPaint = new Paint();
        mBounds = new Rect();
        mTitleHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dp2px(8), getResources().getDisplayMetrics());
        mTitleFontSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, dp2px(4), getResources().getDisplayMetrics());
    ...
    public SuspensionDecoration setMarginLeft(int mMarginLeft) {
    ...
    private void drawTitleArea(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {//最先调用，绘制在最下层
        ...
        c.drawText(mDatas.get(position).getSuspensionTag(), dp2px(child.getLeft() + 8), child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);
    }
    ...
    public void onDrawOver(Canvas c, final RecyclerView parent, RecyclerView.State state) {//最后调用 绘制在最上层
        ...
        c.drawText(tag, dp2px(child.getLeft() + 8), parent.getPaddingTop() + mTitleHeight - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);
        if (flag) {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (position > -1) {
            ...
            if (titleCategoryInterface.isShowSuspension()) {
                if (position == 0) {
                    outRect.set(mMarginLeft, mTitleHeight, 0, 0);
                } else {//其他的通过判断
                    if ((!TextUtils.isEmpty(titleCategoryInterface.getSuspensionTag()) && !titleCategoryInterface.getSuspensionTag().equals(mDatas.get(position - 1).getSuspensionTag()))) {
                        //不为空 且跟前一个tag不一样了，说明是新的分类，也要title
                        outRect.set(mMarginLeft, mTitleHeight, 0, 0);
                    } else {
                        outRect.set(mMarginLeft, 0, 0, 0);
                    }
                }
            }
        }
    ...
    public boolean isInterceptEvent(RecyclerView view, View child, int position) {
        int pos = ((LinearLayoutManager) (view.getLayoutManager())).findFirstVisibleItemPosition();
        pos -= getHeaderViewCount();
        if (pos == position) {
